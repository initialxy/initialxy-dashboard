...
async function genSerializeThrift(thriftObj: {
  ...
  return new Promise((resolve, _) => {
    const trans = new TFramedTransport(
      ...
      (msg?: Buffer, _seqid?: number) => resolve(toArrayBuffer(nullthrows(msg).subarray(4))),
    )
    const protocal = new TBinaryProtocol(trans)
...
export default class API {
  ...
  static async genAddStock(stock: Stock): Promise<Stock> {
    ...
    const respArrayBuffer = await resp.arrayBuffer()
    return onlyx(deserializeThrift(Buffer.from(respArrayBuffer), Stocks).stocks)
  }
  ...
  static async genAddTask(task: Task): Promise<Task> {
    ...
    const resp = await sendThriftObjToAPI(new Tasks({ tasks: [task] }), getTasksEndpoint(), 'POST')
    const respArrayBuffer = await resp.arrayBuffer()
